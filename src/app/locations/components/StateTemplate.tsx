...
import { StateData, SimpleContentSection, SimpleContentSubsection } from '../data/types';
...
// Optimized STATE TEMPLATE BREADCRUMB
const BreadcrumbNav = React.memo(({ stateData }: { stateData: StateData }) => {
  const stateName = stateData.name;
  const maxStateLength = 18;
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1 sm:gap-2 w-full min-w-0">
        <Link 
          href={stateData.ui.breadcrumb.homeLink}
          ...
          prefetch={false}
        >
          {stateData.ui.breadcrumb.homeText}
        </Link>
        <ChevronRight className="h-3 w-3 sm:h-4 sm:w-4 text-slate-400 flex-shrink-0" aria-hidden="true" />

        <Link 
          href={stateData.ui.breadcrumb.locationsLink}
          className="hover:text-blue-600 transition-colors font-medium focus:outline-none focus:ring-1 focus:ring-blue-500 rounded px-1 py-0.5 whitespace-nowrap flex-shrink-0"
          prefetch={false}
        >
          {stateData.ui.breadcrumb.locationsText}
        </Link>
        <ChevronRight className="h-3 w-3 sm:h-4 sm:w-4 text-slate-400 flex-shrink-0" aria-hidden="true" />
...
export default function StateTemplate({ stateData }: StateTemplateProps) {
  ...
  const [servicesToShow, setServicesToShow] = useState(stateData.ui.servicesSection.initialDisplay);
  ...
  const toggleCities = useCallback(() => {
    setCitiesToShow(prev => prev === 16 ? stateData.cities.length : 16);
  }, [stateData.cities.length]);

  // Optimized helper functions
  const replacePlaceholders = useCallback((text: string): string => {
    return text.replace(/{stateName}/g, stateData.name);
  }, [stateData.name]);
  ...
  const displayedServices = useMemo(() => 
    showAllServices ? stateData.services.list : stateData.services.list.slice(0, servicesToShow),
    [showAllServices, stateData.services.list, servicesToShow]
  );

  const displayedCities = useMemo(() => 
    stateData.cities.slice(0, citiesToShow),
    [stateData.cities, citiesToShow]
  );

  const hasSimplifiedContent = useMemo(() => 
    (stateData.content as any).sections && 
    Array.isArray((stateData.content as any).sections) && 
    (stateData.content as any).sections.length > 0 && 
    typeof (stateData.content as any).sections[0].heading2 === 'string',
    [stateData.content]
  );
  ...
  const renderSimpleContentSubsection = useCallback((subsection: SimpleContentSubsection, parentLevel: number = 2) => {
    ...
    if (subsection.heading3) {
      headingLevel = 'h3';
      headingText = replacePlaceholders(subsection.heading3);
    } else if (subsection.heading4) {
      headingLevel = 'h4';
      headingText = replacePlaceholders(subsection.heading4);
    } else if (subsection.heading5) {
      headingLevel = 'h5';
      headingText = replacePlaceholders(subsection.heading5);
    } else if (subsection.heading6) {
      headingLevel = 'h6';
      headingText = replacePlaceholders(subsection.heading6);
    }
    ...
    return (
      <div key={headingText || 'subsection'} className="mb-4 sm:mb-6">
        ...
        <p className="text-sm sm:text-base lg:text-lg text-slate-700 leading-relaxed">
          {replacePlaceholders(subsection.content)}
        </p>
    ...
    return (
      <div key={index} className="space-y-4 sm:space-y-6 lg:space-y-8">
        <h2 className={contentHeadingClasses.h2}>
          {replacePlaceholders(section.heading2)}
        </h2>
  ...
  }, [replacePlaceholders, renderSimpleContentSubsection]);
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 pt-4 sm:pt-6 lg:pt-8">
        <div className="min-h-[2rem] sm:min-h-[2.5rem]">
          <BreadcrumbNav stateData={stateData} />
        </div>
      </div>
          ...
          <div className="grid lg:grid-cols-2 gap-4 sm:gap-6 lg:gap-8 xl:gap-16 items-center">
            ...
            <div className="order-2 lg:order-1">
              ...
              <div className="flex items-center gap-1.5 sm:gap-2 lg:gap-3 mb-3 sm:mb-4 lg:mb-6 xl:mb-8">
                <div className="relative h-4 w-4 sm:h-6 sm:w-6 lg:h-8 lg:w-8 xl:h-12 xl:w-12 2xl:h-16 2xl:w-16 flex-shrink-0">
                  <Image
                    src={stateData.hero.badgeIcon.src}
                    alt={stateData.hero.badgeIcon.alt}
                    fill
                ...
                <div className="flex items-center gap-1 sm:gap-1.5 lg:gap-2 bg-green-50 border border-green-200 rounded-full px-1.5 py-0.5 sm:px-2 sm:py-1 lg:px-3 lg:py-2">
                  <Verified className="h-2.5 w-2.5 sm:h-3 sm:w-3 lg:h-4 lg:w-4 xl:h-6 xl:w-6 text-green-600 flex-shrink-0" />
                  <span className="text-green-700 font-medium text-xs sm:text-sm lg:text-base xl:text-xl leading-tight">
                    {replacePlaceholders(stateData.hero.badge)}
                  </span>
                </div>
              </div>
              ...
              <h1 className="font-medium text-base sm:text-lg lg:text-xl xl:text-2xl 2xl:text-3xl text-slate-900 leading-tight mb-2 sm:mb-3 lg:mb-4 xl:mb-6">
                {replacePlaceholders(stateData.hero.title)}
                {stateData.hero.subtitle && (
                  <span className="block bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent text-sm sm:text-base lg:text-lg xl:text-xl mt-1 sm:mt-1.5 lg:mt-2">
                    {replacePlaceholders(stateData.hero.subtitle)}
                  </span>
                )}
              </h1>
              ...
              <p className="text-xs sm:text-sm lg:text-base xl:text-lg text-slate-700 leading-relaxed mb-3 sm:mb-4 lg:mb-6 xl:mb-8">
                {replacePlaceholders(stateData.hero.description)}
              </p>
              ...
              <div className="mb-3 sm:mb-4 lg:mb-6 xl:mb-8">
                ...
                >
                  ...
                  <span className="whitespace-nowrap">{stateData.ui.heroSection.ctaButton.text}</span>
                  <span className="font-mono font-medium text-xs sm:text-sm lg:text-base whitespace-nowrap">{stateData.contact.phoneDisplay}</span>
                </Link>
              </div>

              {/* Trust Indicators - Simplified */}
              <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 lg:gap-3 xl:gap-4 text-slate-600">
                {stateData.hero.trustIndicators.map((indicator, index) => {
                  ...
                  return (
                    <div key={index} className="flex items-center gap-1 sm:gap-1.5">
                      ...
                      <span className="font-medium text-xs sm:text-sm leading-tight whitespace-nowrap">
                        {replacePlaceholders(indicator.text)}
                      </span>
                    </div>
                  );
                })}
            ...
            <div className="relative order-1 lg:order-2">
              <div className="relative w-full aspect-[5/4] sm:aspect-[4/3] lg:aspect-[3/2] xl:aspect-[4/3] 2xl:aspect-[3/2] rounded-lg sm:rounded-xl lg:rounded-2xl xl:rounded-3xl overflow-hidden shadow-lg sm:shadow-xl lg:shadow-2xl">
                <Image
                  src={stateData.hero.image}
                  alt={stateData.hero.imageAlt}
                  fill
                ...
                <div className="absolute top-1.5 sm:top-2 lg:top-3 xl:top-4 left-1.5 sm:left-2 lg:left-3 xl:left-4 bg-white/90 backdrop-blur-sm rounded-md sm:rounded-lg lg:rounded-xl px-1.5 sm:px-2 lg:px-3 py-1 sm:py-1.5 lg:py-2 shadow-lg">
                  <div className="flex items-center gap-1 sm:gap-1.5 lg:gap-2">
                    ...
                    <span className="font-medium text-xs sm:text-sm text-slate-800 whitespace-nowrap leading-tight">
                      {stateData.ui.heroSection.availabilityBadge.text}
                    </span>
                  </div>
                </div>
              </div>
      ...
      <section className="py-6 sm:py-8 lg:py-12 xl:py-20 bg-white relative">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="space-y-6 sm:space-y-8 lg:space-y-12 xl:space-y-16">
            {(stateData.content as any).introduction && (
              <div className="text-center max-w-4xl mx-auto">
                <p className="text-base sm:text-lg lg:text-xl xl:text-2xl text-slate-700 leading-relaxed">
                  {replacePlaceholders((stateData.content as any).introduction)}
                </p>
              </div>
            )}
            ...
            <div className="space-y-6 sm:space-y-8 lg:space-y-12 xl:space-y-16">
              {hasSimplifiedContent ? (
                (stateData.content as any).sections.map((section: SimpleContentSection, index: number) => (
                  renderSimpleContentSection(section, index)
                ))
              ) : (
                stateData.content.sections?.map((section, index) => (
                  <div key={index} className="space-y-4 sm:space-y-6 lg:space-y-8">
                    <h2 className={contentHeadingClasses.h2}>
                      {replacePlaceholders((section as any).heading2 || (section as any).title || `Section ${index + 1}`)}
                    </h2>
                    
                    {section.subsections && section.subsections.map((subsection, subIndex) => (
                      <div key={subIndex} className="ml-2 sm:ml-4 space-y-3 sm:space-y-4">
                        <h3 className={contentHeadingClasses.h3}>
                          {replacePlaceholders((subsection as any).heading3 || (subsection as any).subtitle || `Subsection ${subIndex + 1}`)}
                        </h3>
                        <p className="text-sm sm:text-base lg:text-lg text-slate-700 leading-relaxed">
                          {replacePlaceholders(subsection.content)}
                        </p>
          ...
          <div className="text-center">
            <div className="max-w-4xl mx-auto">
              <div className="bg-gradient-to-r from-blue-50 to-purple-50 border border-blue-200 rounded-xl sm:rounded-2xl p-4 sm:p-6">
                <h3 className="font-medium text-lg sm:text-xl text-slate-800 mb-2 sm:mb-3">{stateData.ui.ctaSections.afterContent.title}</h3>
                <Link 
                  href={`tel:${stateData.contact.phone}`}
                ...
                >
                  ...
                  <span className="whitespace-nowrap">{stateData.ui.ctaSections.afterContent.buttonText}</span>
                  <span className="font-mono font-medium whitespace-nowrap">{stateData.contact.phoneDisplay}</span>
                </Link>
              </div>
            </div>
          </div>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center mb-6 sm:mb-8 lg:mb-12 xl:mb-16">
            <div className="inline-flex items-center bg-gradient-to-r from-blue-100 to-purple-100 border border-blue-200 text-blue-800 px-4 sm:px-6 py-2 sm:py-3 rounded-full font-medium text-xs sm:text-sm shadow-sm mb-4 sm:mb-6">
              {React.createElement(getIconComponent(stateData.ui.servicesSection.badge.icon), {
                className: "h-4 w-4 sm:h-5 sm:w-5 mr-2"
              })}
              {replacePlaceholders(stateData.ui.servicesSection.badge.text)}
            </div>
            <h2 className="font-medium text-xl sm:text-2xl lg:text-3xl xl:text-4xl text-slate-900 mb-4 sm:mb-6 leading-tight">
              {replacePlaceholders(stateData.services.title)}
              <span className="block bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">
                {replacePlaceholders(stateData.services.subtitle)}
              </span>
            </h2>
            <p className="text-sm sm:text-base lg:text-lg xl:text-xl text-slate-700 max-w-3xl mx-auto leading-relaxed">
              {replacePlaceholders(stateData.services.description)}
            </p>
          </div>
            ...
            >
              {showAllServices ? (
                <>
                  ...
                  {stateData.ui.servicesSection.showLessButton.text}
                </>
              ) : (
                <>
                  <div className="flex items-center justify-center w-6 h-6 sm:w-7 sm:h-7 bg-blue-600 rounded-full text-white">
                  ...
                  {stateData.ui.servicesSection.showMoreButton.text}
                </>
              )}
            </button>
          </div>
            ...
            <div className="bg-gradient-to-br from-blue-50 via-slate-50 to-purple-50 rounded-2xl sm:rounded-3xl p-6 sm:p-8 lg:p-12 shadow-sm relative overflow-hidden max-w-4xl mx-auto">
              <div className="relative z-10 text-center">
                <div className="inline-flex items-center bg-white/90 border border-blue-200 rounded-full px-4 sm:px-6 py-2 sm:py-3 mb-4 sm:mb-6">
                  {React.createElement(getIconComponent(stateData.ui.ctaSections.afterServices.badge.icon), {
                    className: "h-4 w-4 sm:h-5 sm:w-5 mr-2 text-blue-600"
                  })}
                  <span className="font-medium text-blue-800 text-xs sm:text-sm">{stateData.ui.ctaSections.afterServices.badge.text}</span>
                </div>
                
                <h3 className="font-medium text-lg sm:text-xl text-slate-800 mb-3 sm:mb-4">
                  {stateData.ui.ctaSections.afterServices.title}
                </h3>
                
                <p className="text-slate-600 mb-6 sm:mb-8 max-w-2xl mx-auto text-base sm:text-lg leading-relaxed">
                  {stateData.ui.ctaSections.afterServices.description}
                </p>
                ...
                <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center">
                  ...
                  >
                    ...
                    <span className="whitespace-nowrap">{stateData.ui.ctaSections.afterServices.primaryButton.text}</span>
                    <span className="font-mono font-medium whitespace-nowrap">{stateData.contact.phoneDisplay}</span>
                    <ArrowRight className="h-3 w-3 sm:h-4 sm:w-4 group-hover:translate-x-1 transition-transform" />
                  </Link>
                  
                  <Link 
                    href={stateData.ui.ctaSections.afterServices.secondaryButton.link}
                  ...
                  >
                    {React.createElement(getIconComponent(stateData.ui.ctaSections.afterServices.secondaryButton.icon), {
                      className: "h-4 w-4 sm:h-5 sm:w-5"
                    })}
                    {stateData.ui.ctaSections.afterServices.secondaryButton.text}
                    <ArrowRight className="h-3 w-3 sm:h-4 sm:w-4 group-hover:translate-x-1 transition-transform" />
                  </Link>
                </div>
              </div>
      ...
      <section className="py-6 sm:py-8 lg:py-12 xl:py-20 bg-white relative">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center mb-6 sm:mb-8 lg:mb-12 xl:mb-16">
            <div className="inline-flex items-center bg-gradient-to-r from-blue-100 to-purple-100 border border-blue-200 text-blue-800 px-4 sm:px-6 py-2 sm:py-3 rounded-full font-medium text-xs sm:text-sm shadow-sm mb-4 sm:mb-6">
              {React.createElement(getIconComponent(stateData.ui.whyChooseUsSection.badge.icon), {
                className: "h-4 w-4 sm:h-5 sm:w-5 mr-2"
              })}
              {stateData.ui.whyChooseUsSection.badge.text}
            </div>
            <h2 className="font-medium text-xl sm:text-2xl lg:text-3xl xl:text-4xl text-slate-900 mb-4 sm:mb-6 leading-tight">
              {replacePlaceholders(stateData.whyChooseUs.title)}
              <span className="block bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">
                {replacePlaceholders(stateData.whyChooseUs.subtitle)}
              </span>
            </h2>
            <p className="text-sm sm:text-base lg:text-lg xl:text-xl text-slate-700 max-w-3xl mx-auto leading-relaxed">
              {replacePlaceholders(stateData.whyChooseUs.description)}
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
            {stateData.whyChooseUs.reasons.map((reason, index) => (
              <div key={index} className="bg-white/90 backdrop-blur-sm rounded-xl sm:rounded-2xl p-4 sm:p-6 lg:p-8 shadow-sm hover:shadow-md transition-all duration-100 transform hover:-translate-y-2 text-center border border-slate-200 hover:border-slate-300">
                <div className="inline-block mb-4 sm:mb-6">
                  {reason.iconPath ? (
                    <div className="relative h-12 w-12 sm:h-14 sm:w-14 lg:h-16 lg:w-16 mx-auto">
                      <Image
                        src={reason.iconPath}
                        alt={replacePlaceholders(reason.title)}
                        fill
                        className="object-contain"
                ...
                <h3 className="font-medium text-base sm:text-lg lg:text-xl text-slate-800 mb-3 sm:mb-4">
                  {replacePlaceholders(reason.title)}
                </h3>
                <p className="text-sm sm:text-base lg:text-lg xl:text-xl text-slate-600 leading-relaxed">
                  {replacePlaceholders(reason.description)}
                </p>
              </div>
            ))}
          </div>
      ...
      <section className="py-6 sm:py-8 lg:py-12 xl:py-20 bg-white relative">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center mb-6 sm:mb-8 lg:mb-12 xl:mb-16">
            <div className="inline-flex items-center bg-gradient-to-r from-blue-100 to-purple-100 border border-blue-200 text-blue-800 px-4 sm:px-6 py-2 sm:py-3 rounded-full font-medium text-xs sm:text-sm shadow-sm mb-4 sm:mb-6">
              {React.createElement(getIconComponent(stateData.ui.reviewsSection.badge.icon), {
                className: "h-4 w-4 sm:h-5 sm:w-5 mr-2"
              })}
              {replacePlaceholders(stateData.ui.reviewsSection.badge.text)}
            </div>
            <h2 className="font-medium text-xl sm:text-2xl lg:text-3xl xl:text-4xl text-slate-900 mb-4 sm:mb-6 leading-tight">
              {stateData.ui.reviewsSection.title.part1} {stateData.name} {stateData.ui.reviewsSection.title.part2}
              <span className="block bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">
                {stateData.ui.reviewsSection.title.part3}
              </span>
            </h2>
            <p className="text-sm sm:text-base lg:text-lg xl:text-xl text-slate-700 mb-6 sm:mb-8 max-w-3xl mx-auto leading-relaxed">
              {replacePlaceholders(stateData.ui.reviewsSection.description)}
            </p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8 mb-6 sm:mb-8 lg:mb-12">
            {stateData.testimonials.map((review, index) => (
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {review.verified && (
                      <span className="bg-green-100 text-green-700 font-medium text-xs px-2 py-1 rounded-full whitespace-nowrap">
                        {stateData.ui.reviewsSection.verifiedBadge.text}
                      </span>
                    )}
                  </div>
                </div>
          ...
          <div className="text-center">
            <div className="inline-flex items-center gap-4 sm:gap-6 lg:gap-8 bg-white/80 backdrop-blur-sm rounded-xl sm:rounded-2xl px-4 sm:px-6 lg:px-8 py-4 sm:py-6 shadow-lg border border-blue-200 flex-wrap justify-center">
              {stateData.trustIndicators.map((indicator, index) => (
      ...
      <section className="py-8 sm:py-12 lg:py-16 bg-white">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <div className="max-w-4xl mx-auto">
              <div className="bg-gradient-to-r from-blue-50 to-purple-50 border border-blue-200 rounded-xl sm:rounded-2xl p-4 sm:p-6">
                <h3 className="font-medium text-lg sm:text-xl text-slate-800 mb-2 sm:mb-3">{stateData.ui.ctaSections.afterReviews.title}</h3>
                <Link 
                  href={`tel:${stateData.contact.phone}`}
                  className="inline-flex items-center bg-gradient-to-r from-blue-100 to-purple-100 border border-blue-200 text-blue-800 hover:from-blue-200 hover:to-purple-200 px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg sm:rounded-xl font-medium transition-all gap-2 text-sm sm:text-base"
                >
                  <Phone className="h-4 w-4" />
                  <span className="whitespace-nowrap">{stateData.ui.ctaSections.afterReviews.buttonText}</span>
                  <span className="font-mono font-medium whitespace-nowrap">{stateData.contact.phoneDisplay}</span>
                </Link>
              </div>
            </div>
      ...
      <section className="py-6 sm:py-8 lg:py-12 xl:py-20 bg-white relative">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center mb-6 sm:mb-8 lg:mb-12 xl:mb-16">
            <div className="inline-flex items-center bg-gradient-to-r from-blue-100 to-purple-100 border border-blue-200 text-blue-800 px-4 sm:px-6 py-2 sm:py-3 rounded-full font-medium text-xs sm:text-sm shadow-sm mb-4 sm:mb-6">
              {React.createElement(getIconComponent(stateData.ui.processSection.badge.icon), {
                className: "h-4 w-4 sm:h-5 sm:w-5 mr-2"
              })}
              {stateData.ui.processSection.badge.text}
            </div>
            <h2 className="font-medium text-xl sm:text-2xl lg:text-3xl xl:text-4xl text-slate-900 mb-4 sm:mb-6 leading-tight">
              {replacePlaceholders(stateData.process.title)}
              <span className="block bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">
                {replacePlaceholders(stateData.process.subtitle)}
              </span>
            </h2>
            <p className="text-sm sm:text-base lg:text-lg xl:text-xl text-slate-700 max-w-3xl mx-auto leading-relaxed">
              {replacePlaceholders(stateData.process.description)}
            </p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 lg:gap-8">
            {stateData.process.steps.map((step, index) => (
              <div key={step.step} className="text-center bg-white/90 rounded-xl sm:rounded-2xl p-4 sm:p-6 lg:p-8 shadow-lg hover:shadow-xl transition-all border border-blue-200">
                <div className="flex items-center justify-center mx-auto mb-4 sm:mb-6">
                  {step.iconPath ? (
                    <div className="relative h-12 w-12 sm:h-14 sm:w-14 lg:h-16 lg:w-16">
                      <Image
                        src={step.iconPath}
                        alt={replacePlaceholders(step.title)}
                        fill
                        className="object-contain"
                        sizes="(max-width: 640px) 48px, (max-width: 1024px) 56px, 64px"
                        loading="lazy"
                ...
                <div className="text-blue-600 font-medium mb-2 text-xs sm:text-sm">{stateData.ui.processSection.stepLabel} {step.step}</div>
                <h3 className="font-medium text-base sm:text-lg lg:text-xl text-slate-800 mb-3 sm:mb-4">
      ...
      <section className="py-6 sm:py-8 lg:py-12 xl:py-20 bg-white relative">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center mb-6 sm:mb-8 lg:mb-12 xl:mb-16">
            <div className="inline-flex items-center bg-gradient-to-r from-blue-100 to-purple-100 border border-blue-200 text-blue-800 px-4 sm:px-6 py-2 sm:py-3 rounded-full font-medium text-xs sm:text-sm shadow-sm mb-4 sm:mb-6">
              {React.createElement(getIconComponent(stateData.ui.citiesSection.badge.icon), {
                className: "h-4 w-4 sm:h-5 sm:w-5 mr-2"
              })}
              {replacePlaceholders(stateData.ui.citiesSection.badge.text)}
            </div>
            <h2 className="font-medium text-xl sm:text-2xl lg:text-3xl xl:text-4xl text-slate-900 mb-4 sm:mb-6 leading-tight">
              {stateData.ui.citiesSection.title.part1} {stateData.cities.length}+ {stateData.ui.citiesSection.title.part2}
              <span className="block bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">
                {stateData.ui.citiesSection.title.part3} {stateData.name}
              </span>
            </h2>
            <p className="text-sm sm:text-base lg:text-lg xl:text-xl text-slate-700 mb-6 sm:mb-8 max-w-3xl mx-auto leading-relaxed">
              {replacePlaceholders(stateData.ui.citiesSection.description)}
            </p>
          </div>
            ...
            >
              {citiesToShow === 16 ? (
                <>
                  ...
                  {stateData.ui.citiesSection.showAllButton.text}
                </>
              ) : (
                <>
                  <ChevronUp className="h-4 w-4 sm:h-5 sm:w-5" />
                  {stateData.ui.citiesSection.showLessButton.text}
                </>
              )}
            </button>
          </div>
      ...
      <section className="py-6 sm:py-8 lg:py-12 xl:py-20 bg-white relative">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl relative z-10">
          <div className="text-center mb-6 sm:mb-8 lg:mb-12 xl:mb-16">
            <div className="inline-flex items-center bg-gradient-to-r from-blue-100 to-purple-100 border border-blue-200 text-blue-800 px-4 sm:px-6 py-2 sm:py-3 rounded-full font-medium text-xs sm:text-sm shadow-sm mb-4 sm:mb-6">
              {React.createElement(getIconComponent(stateData.ui.faqSection.badge.icon), {
                className: "h-4 w-4 sm:h-5 sm:w-5 mr-2"
              })}
              {replacePlaceholders(stateData.ui.faqSection.badge.text)}
            </div>
            <h2 className="font-medium text-xl sm:text-2xl lg:text-3xl xl:text-4xl text-slate-900 mb-4 sm:mb-6 leading-tight">
              {stateData.name}-{stateData.ui.faqSection.title.suffix}
              <span className="block bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">
                {stateData.ui.faqSection.title.highlight}
              </span>
            </h2>
            <p className="text-sm sm:text-base lg:text-lg xl:text-xl text-slate-700 mb-6 sm:mb-8 max-w-3xl mx-auto leading-relaxed">
              {replacePlaceholders(stateData.ui.faqSection.description)}
            </p>
          </div>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="max-w-4xl mx-auto">
            <div className="bg-gradient-to-br from-blue-50 via-slate-50 to-purple-50 rounded-2xl sm:rounded-3xl p-6 sm:p-8 lg:p-12 shadow-sm relative overflow-hidden">
              <div className="relative z-10 text-center">
                <div className="inline-flex items-center bg-white/90 border border-blue-200 rounded-full px-4 sm:px-6 py-2 sm:py-3 mb-4 sm:mb-6">
                  {React.createElement(getIconComponent(stateData.ui.ctaSections.beforeMap.badge.icon), {
                    className: "h-4 w-4 sm:h-5 sm:w-5 mr-2 text-blue-600"
                  })}
                  <span className="font-medium text-blue-800 text-xs sm:text-sm">
                    {stateData.ui.ctaSections.beforeMap.badge.text}
                  </span>
                </div>
                
                <h3 className="font-medium text-lg sm:text-xl lg:text-2xl xl:text-3xl text-slate-800 mb-3 sm:mb-4">
                  {replacePlaceholders(stateData.ui.ctaSections.beforeMap.title)}
                </h3>
                
                <p className="text-slate-600 mb-6 sm:mb-8 max-w-2xl mx-auto text-base sm:text-lg leading-relaxed">
                  {replacePlaceholders(stateData.ui.ctaSections.beforeMap.description)}
                </p>
                ...
                <div className="flex justify-center mb-4 sm:mb-6">
                  ...
                  >
                    <Phone className="h-4 w-4 sm:h-5 sm:w-5" />
                    <span className="whitespace-nowrap">{replacePlaceholders(stateData.ui.ctaSections.beforeMap.buttonText)}</span>
                    <ArrowRight className="h-3 w-3 sm:h-4 sm:w-4 group-hover:translate-x-1 transition-transform" />
                  </Link>
                </div>

                <div className="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-6 text-xs sm:text-sm text-slate-600">
                  {stateData.ui.ctaSections.beforeMap.features.map((feature, index) => (
                    <div key={index} className="flex items-center gap-2">
                      <CheckCircle className="h-3 w-3 sm:h-4 sm:w-4 text-green-500" />
                      <span>{replacePlaceholders(feature.text)}</span>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <section className="py-6 sm:py-8 lg:py-12 xl:py-20 bg-white relative">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center mb-6 sm:mb-8 lg:mb-12">
            <div className="inline-flex items-center bg-gradient-to-r from-blue-100 to-purple-100 border border-blue-200 text-blue-800 px-4 sm:px-6 py-2 sm:py-3 rounded-full font-medium text-xs sm:text-sm shadow-sm mb-4 sm:mb-6">
              {React.createElement(getIconComponent(stateData.ui.mapSection.badge.icon), {
                className: "h-4 w-4 sm:h-5 sm:w-5 mr-2"
              })}
              {stateData.ui.mapSection.badge.text}
            </div>
            <h2 className="font-medium text-xl sm:text-2xl lg:text-3xl xl:text-4xl text-slate-900 mb-4 sm:mb-6 leading-tight">
              {stateData.ui.mapSection.title}
              <span className="block bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">
                {stateData.ui.mapSection.subtitle}
              </span>
            </h2>
            <p className="text-sm sm:text-base lg:text-lg xl:text-xl text-slate-700 max-w-3xl mx-auto leading-relaxed">
              {replacePlaceholders(stateData.ui.mapSection.description)}
            </p>
          </div>
          ...
          <div className="rounded-2xl sm:rounded-3xl overflow-hidden shadow-xl sm:shadow-2xl max-w-6xl mx-auto border border-slate-200">
            <iframe
              ...
              title={`${stateData.name} Service Coverage Map`}
